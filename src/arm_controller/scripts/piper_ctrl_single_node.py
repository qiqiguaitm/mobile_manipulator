...
from arm_planner.msg import PiperStatusMsg, PosCmd, PiperEulerPose
from arm_planner.srv import Enable, EnableResponse
from arm_planner.srv import Gripper, GripperResponse
from arm_planner.srv import GoZero, GoZeroResponse
...
class C_PiperRosNode():
    ...
    def joint_callback(self, joint_data):
        ...
        if not self.block_ctrl_flag:
            ...
            # 检查是否只发送了夹爪数据（只有joint7和joint8）
            if len(joint_data.name) == 2 and 'joint7' in joint_data.name and 'joint8' in joint_data.name:
                # 只处理夹爪
                if self.GetEnableFlag() and self.gripper_exist:
                    # 获取joint7的索引
                    joint7_idx = joint_data.name.index('joint7')
                    joint_6 = round(joint_data.position[joint7_idx]*1000*1000)
                    ...
                    if(len(joint_data.effort) > joint7_idx):
                        gripper_effort = joint_data.effort[joint7_idx]
                ...
                return  # 只处理夹爪，不继续执行后续代码
            
            # 原有的完整关节处理逻辑
            if len(joint_data.position) < 6:
                rospy.logwarn("Received incomplete joint state data")
                return
