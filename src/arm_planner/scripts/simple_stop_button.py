...
class SimpleStopButton:
    ...
    def _init_moveit_with_retry(self):
        ...
        while retry_count < max_retries and not rospy.is_shutdown():
            try:
                self.arm_group = moveit_commander.MoveGroupCommander("piper_with_gripper")
                rospy.loginfo("MoveGroup 'piper_with_gripper' initialized successfully")
                return
            except RuntimeError as e:
                retry_count += 1
                rospy.logwarn(f"MoveGroup 'piper_with_gripper' not ready, retry {retry_count}/{max_retries}: {e}")
                rospy.sleep(1.0)
        
        if self.arm_group is None:
            rospy.logerr("Failed to initialize MoveGroup 'piper_with_gripper' after maximum retries")
